...
public class OracleProdutoDAO implements ProdutoDAO{
	...
	public void cadastrar(Produto produto) throws DBException {
		...
		try {
			...
			String sql = "INSERT INTO TB_PRODUTO (CD_PRODUTO, NM_PRODUTO, QT_PRODUTO, "
					+ "VL_PRODUTO, DT_FABRICACAO, CD_CATEGORIA) VALUES (SQ_TB_PRODUTO.NEXTVAL, "
					+ "?, ?, ?, ?, ?)";
	...
	public void atualizar(Produto produto) throws DBException {
		...
		try {
			connection = ConnectionManager.getInstance().getConnection();
			String sql = "UPDATE TB_PRODUTO SET NM_PRODUTO = ?, QT_PRODUTO = ?, "
					+ "VL_PRODUTO = ?, DT_FABRICACAO = ?,"
	...
	public Produto buscar(int id) {
		...
		try {
			connection = ConnectionManager.getInstance().getConnection();
			stmt = connection.prepareStatement("SELECT * FROM TB_PRODUTO INNER JOIN TB_CATEGORIA ON TB_PRODUTO.CD_CATEGORIA = TB_CATEGORIA.CD_CATEGORIA WHERE TB_PRODUTO.CD_PRODUTO = ?");
			stmt.setInt(1, id);
			...
			if (rs.next()){
				...
				produto = new Produto(codigo, nome, valor, dataFabricacao, quantidade);
	...
	public List<Produto> listar() {
		...
		try {
			connection = ConnectionManager.getInstance().getConnection();
			stmt = connection.prepareStatement("SELECT * FROM TB_PRODUTO INNER JOIN"
					+ " TB_CATEGORIA ON TB_PRODUTO.CD_CATEGORIA = TB_CATEGORIA.CD_CATEGORIA");
			rs = stmt.executeQuery();
